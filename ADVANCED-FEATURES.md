...
Every chapter is now a real document at its own URL, generated by the build from the single authored `index.html`. The home document keeps instant in-page switching: internal links are real addresses that the runtime intercepts and records with the History API, exactly the crawlable-link pattern Google's JavaScript SEO guidance recommends. Old `#hash` links still resolve.

- Each chapter document carries only its own chapter plus the shared shell, its own title/description/canonical/Open Graph metadata, and BreadcrumbList (plus FAQPage on Admissions) structured data.
- `sitemap.xml` lists all eight addresses, `robots.txt` references it, and a branded `404.html` links every page.
- Shared scripts resolve assets, fonts, the manifest, service worker and optional motion files from the bundle's own location, so everything works one directory deeper; lazy photographs and icons are pinned to absolute URLs before any address change so they never re-resolve against the wrong chapter.
- Site search and the visit planner navigate to the correct page when a destination lives in another chapter.
- The opt-in offline worker precaches all eight documents (cache `mamss-public-v10`).
- Tests: `tests/pages.cjs` — 24 real-browser checks covering documents, metadata, link resolution, SPA and full navigations, accessibility, no-JavaScript rendering, offline chapter loads and build determinism.
