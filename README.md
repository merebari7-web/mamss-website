# MAMSS — school website, version 2
...
npm test            # 60 real-browser regression checks
```
...
| `site.css`, `site.js` | New design system, responsive layout, routing, focus/menu management, admission handoff and optional effects loader |
...
| `tests/site.cjs` | Browser regression and automated accessibility checks |
| `lighthouserc.cjs`, `.github/workflows/` | Automated tests and deployed-site Lighthouse monitoring |

## Verification

The version-2 regression suite contains **60 passing checks** in Chromium. It covers:
...
The original live mobile Lighthouse baseline was **92 / 100 / 100 / 100** (performance/accessibility/best practices/SEO). A compressed local version-2 run measured **94 / 100 / 100 / 100**. These are individual lab runs in different hosting conditions, not a promised real-world speed increase. The Lighthouse workflow waits for the matching deployed HTML, CSS, JS and worker, runs two production audits and uploads the reports. Thresholds are 85 performance and 95 for the other categories to catch regressions without promising perfect scores.
...
GitHub Pages serves `main` at the repository root; `.nojekyll` preserves the static structure. Keep all deployed relative paths intact. Bump the version in `sw.js` when releasing a changed offline shell, rebuild, run the suite, and verify the live deployment. The hosting provider controls HTTPS, compression and HTTP security headers.
